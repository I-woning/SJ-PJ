...
io.on('connection', (socket) => {
    ...
    socket.on('start_game', () => {
        ...
        broadcastRoomState(roomId); // 파티원 정보 즉시 동기화
        broadcastRoomList();
    ...
    if (gs.isWaitingForInput) {
        if (gs.phase === 'COMBAT') {
            ...
            if (currentActor) {
                ...
                // [수정] 전투 중에는 이전 스토리 플레이스홀더를 무시합니다.
                const placeholder = `▶ [${currentActor.name} 행동] 공격 [적], 방어, ${hint}`;
    ...
    try {
        ...
        if (aliveEnemies.some(e => e.name === '태자귀')) {
            ...
            if (triggerPhase > 0) {
                ...
                gameState.incantationIndex = 0;
                const incantations = [null, "사불범정 만마항복", "금성철벽 천라지망", "파사현정 명경지수", "권선징악 인과응보"];
                gameState.currentIncantation = incantations[triggerPhase].replace(/ /g, '');
                let alertMsg = "";
    ...
    try {
        ...
        if (enemy.name === '굶주린 폴터가이스트') {
            ...
            if (rand < 0.45) {
                ...
                } else broadcastRoomLog(roomId, `🌀 ${enemy.name}의 정신 공격을 ${t.name}이 간신히 버텨냈습니다!`, "combat-msg");
            } else if (rand < 0.9) {
                if (Math.random() < 0.8) {
                    t.status.push('공포');
                    t.fearTurns = 0;
                    broadcastRoomLog(roomId, `😨 ${enemy.name}가 영혼을 얼려버리는 환각을 보여줍니다! (공포)`, "combat-msg");
                } else broadcastRoomLog(roomId, `😰 ${enemy.name}가 보여준 공포를 ${t.name}이 정신력으로 이겨냈습니다!`, "combat-msg");
            } else {
            ...
            if (charmTargets.length === 0) broadcastRoomLog(roomId, `💖 ${enemy.name}가 공허한 눈길로 이미 타락한 파티원들을 바라보며 비웃습니다.`, "combat-msg");
            else {
                ...
                actualTargets.forEach(t => {
                    if (Math.random() < 0.66 && t.job !== '사제') {
                        t.status.push('매혹');
                        broadcastRoomLog(roomId, `💖 ${enemy.name}의 원념이 섞인 울음이 ${t.name}을 지배합니다! (매혹)`, "combat-msg");
                    } else if (t.job === '사제') broadcastRoomLog(roomId, `✨ ${t.name}은 성수로 무장한 신성한 기운으로 ${enemy.name}의 유혹을 물리칩니다!`, "guide-msg");
                    else broadcastRoomLog(roomId, `💨 ${enemy.name}의 울음소리가 공허하게 울려 퍼집니다...`, "combat-msg");
                });
            }
            ...
            if (enemy.isCharging) {
                ...
                alive.forEach(t => {
                    let baseDmg = Math.floor(Math.random() * 41) + 40;
                    let finalDmg = baseDmg;
                    if (t.status.includes('방어')) {
                        finalDmg = Math.floor(baseDmg * 0.2);
                        broadcastRoomLog(roomId, `🛡️ ${t.name}이(가) 방어로 충격을 견뎌냅니다! (-${finalDmg})`, "combat-msg");
                        t.status = t.status.filter(s => s !== '방어');
                    } else broadcastRoomLog(roomId, `💥 ${t.name}이(가) 엄청난 충격에 휩싸입니다! (-${finalDmg})`, "combat-msg");
                ...
                if (enemy.chargeCount >= 2) {
                    ...
                    let dmg = Math.floor(Math.random() * 21) + 20;
            ...
            targets.forEach(t => {
                let dmg = calcDmg(Math.floor(Math.random() * 4) + 7, t);
                t.hp = Math.max(0, t.hp - dmg);
...
function parseCommand(input) {
    ...
    if (t === '탐색' || actionStr === '탐색') return { action: 'SEARCH', target: targetStr || 'NONE' };
...
function executePlayerAction(roomId, actor, cmd, socket) {
    ...
    try {
        if (cmd.action === 'LOOK' && gs.phase === 'COMBAT') {
            ...
            if (gs.location === '2층 안방') broadcastRoomLog(roomId, "안방에는 **[거울]**, **[옷장]**, **[침대]** 등이 보입니다. 본체는 거울 속에 있을지 모릅니다.", "guide-msg");
            else if (gs.location === '1층 주방 (식당)') {
                gs.poltergeistState.lookCount++;
        ...
        if (gs.location === '1층 주방 (식당)' && gs.phase === 'COMBAT') {
            if (cmd.action === 'SEARCH') {
                ...
                if (cmd.target === gs.poltergeistState.hiddenSpot) {
                    ...
                    if (p) { p.status = p.status.filter(s => s !== '은신'); broadcastRoomLog(roomId, `✨ 찾았다! **[${cmd.target}]** 속에 숨어있던 폴터가이스트가 모습을 드러냅니다!`, "system-msg"); }
                } else broadcastRoomLog(roomId, `💨 **[${cmd.target}]**에는 아무것도 없었습니다...`, "combat-msg");
                safeNextTurn(roomId, 1000);
                return;
            }
        }
        ...
        if (gs.location === '2층 안방' && gs.phase === 'COMBAT') {
            if (cmd.action === 'SEARCH') {
                if (cmd.target === '거울') {
                    ...
                    if (mihok) { mihok.status = mihok.status.filter(s => s !== '은신'); broadcastRoomLog(roomId, `✨ 거울 속을 샅샅이 뒤져 숨어있던 **[${mihok.name}]**의 본체를 찾아냈습니다!`, "system-msg"); }
                    else broadcastRoomLog(roomId, "거울 속에는 기괴하게 뒤틀린 파티원들의 모습만 비칩니다.", "guide-msg");
                } else broadcastRoomLog(roomId, "무엇을 탐색할 지 몰라 허둥지둥 댑니다. 무언가를 탐색 해야합니다.", "guide-msg");
                safeNextTurn(roomId, 1000);
                return;
            }
        }
        ...
        if (cmd.action === 'ATTACK') {
            ...
            if (!t) { gs.isWaitingForInput = true; socket.emit('game_error', `대상을 찾을 수 없습니다: ${cmd.target}`); io.to(roomId).emit('turn_start', actor); return; }
            const isTK = gs.party.some(p => p.id === t.id);
            let dmg = Math.floor(Math.random() * 10) + 15;
            if (actor.name === '강림' && actor.bonusDmg) dmg += actor.bonusDmg;
            if (t.status.includes('방어')) { dmg = Math.floor(dmg * 0.5); t.status = t.status.filter(s => s !== '방어'); }
            if (t.status.includes('도발방어')) { dmg = Math.floor(dmg * 0.3); t.status = t.status.filter(s => s !== '도발방어'); }
            if (gs.coatBonusPlayerId === t.id) { dmg = Math.max(1, dmg - 4); broadcastRoomLog(roomId, `🧥 코트의 보호로 피해가 감소합니다. (-4)`, "guide-msg"); }
            if (isTK) {
                const canBeWoken = t.status.includes('혼란') || t.status.includes('공포');
                if (canBeWoken) { dmg = Math.floor(Math.random() * 6) + 5; t.status = t.status.filter(s => s !== '혼란' && s !== '공포'); t.fearTurns = 0; broadcastRoomLog(roomId, `💥 ${actor.name}이 ${t.name}을 때려 정신을 차리게 합니다! (-${dmg})`, "combat-msg"); }
                else if (t.status.includes('매혹')) { dmg = Math.floor(Math.random() * 6) + 5; broadcastRoomLog(roomId, `⚠️ ${actor.name}이 매혹된 ${t.name}을 때렸지만 소용없습니다! (-${dmg})`, "combat-msg"); }
                else broadcastRoomLog(roomId, `⚠️ ${actor.name}이 팀원 ${t.name}을 가차없이 공격했습니다! (-${dmg})`, "combat-msg");
            } else if (t.status.includes('은신')) { broadcastRoomLog(roomId, `❌ 은신 중인 **[${t.name}]**을(를) 찾지 못해 공격이 빗나갔습니다!`, "combat-msg"); dmg = 0; }
            else broadcastRoomLog(roomId, `⚔️ ${actor.name}의 공격! ${t.name}에게 ${dmg} 피해!`);
            t.hp = Math.max(0, t.hp - dmg);
        } else if (cmd.action === 'SKILL') {
            const SKILL_COSTS = { '징치기': 10, '사인검베기': 15, '혼령묶기': 20, '성수뿌리기': 15 };
            const SKILL_JOB_MAP = { '징치기': '무당', '사인검베기': '퇴마사', '혼령묶기': '영매', '성수뿌리기': '사제' };
            const requiredJob = SKILL_JOB_MAP[cmd.skillName];
            if (requiredJob && actor.job !== requiredJob) { gs.isWaitingForInput = true; const JOB_NAME_MAP = { '무당': '천화', '퇴마사': '강림', '영매': '연화', '사제': '요한' }; socket.emit('game_error', `${cmd.skillName}은(는) ${JOB_NAME_MAP[requiredJob]} 전용 스킬입니다!`); io.to(roomId).emit('turn_start', actor); return; }
            const cost = SKILL_COSTS[cmd.skillName] || 0;
            if (actor.mp < cost) { gs.isWaitingForInput = true; socket.emit('game_error', `MP가 부족합니다! (필요 MP: ${cost})`); io.to(roomId).emit('turn_start', actor); return; }
            actor.mp -= cost;

            if (cmd.skillName === '성수뿌리기') {
                const t = gs.party.find(p => p.name === cmd.target) || gs.enemies.find(e => e.name === cmd.target);
                if (!t) { gs.isWaitingForInput = true; socket.emit('game_error', '대상을 찾을 수 없습니다.'); io.to(roomId).emit('turn_start', actor); return; }
                if (gs.party.includes(t)) {
                    t.hp = Math.min(t.maxHp, t.hp + 30);
                    const hasCharm = t.status.includes('매혹');
                    t.status = t.status.filter(s => s !== '매혹');
                    if (hasCharm) broadcastRoomLog(roomId, `✨ 요한의 성수! ${t.name}의 매혹이 풀리고 정신을 차립니다! (+30 HP)`, "heal-msg");
                    else broadcastRoomLog(roomId, `✨ 요한의 성수! ${t.name}의 HP가 30 회복됩니다.`, "heal-msg");
                } else { t.hp = Math.max(0, t.hp - 20); broadcastRoomLog(roomId, `💦 요한이 던진 성수가 ${t.name}에게 정화의 고통을 줍니다! (-20)`, "combat-msg"); }
            } else if (cmd.skillName === '사인검베기') {
                const t = gs.enemies.find(e => e.name === cmd.target && e.hp > 0);
                if (!t) { gs.isWaitingForInput = true; socket.emit('game_error', '적을 지정하세요.'); io.to(roomId).emit('turn_start', actor); return; }
                if (t.status.includes('은신')) { broadcastRoomLog(roomId, `❌ 은신 중인 **[${t.name}]**을(를) 찾지 못해 기술이 허공을 가릅니다!`, "combat-msg"); safeNextTurn(roomId, 1000); return; }
                let dmg = 45; if (actor.name === '강림' && actor.bonusDmg) dmg += actor.bonusDmg;
                t.hp = Math.max(0, t.hp - dmg); broadcastRoomLog(roomId, `🗡️ 강림의 사인검이 빛을 뿜으며 ${t.name}을 베어 가릅니다! (-${dmg})`, "combat-msg");
            } else if (cmd.skillName === '혼령묶기') {
                const t = gs.enemies.find(e => e.name === cmd.target && e.hp > 0);
                if (!t) { gs.isWaitingForInput = true; socket.emit('game_error', '적을 지정하세요.'); io.to(roomId).emit('turn_start', actor); return; }
                if (t.status.includes('은신')) { broadcastRoomLog(roomId, `❌ 은신 중인 **[${t.name}]**에게 혼령을 보냈으나 실체를 찾지 못했습니다!`, "combat-msg"); safeNextTurn(roomId, 1000); return; }
                t.status.push('기절');
                t.status.push('묶임'); // 폴터가이스트 기믹 저지용 추가 상태
                broadcastRoomLog(roomId, `🕸️ 연화가 부리는 혼령들이 ${t.name}의 움직임을 완전히 봉쇄합니다! (기절)`, "combat-msg");
                if (t.name === '머리 없는 거구의 악귀' && t.isCharging) {
                    const backlash = Math.floor(Math.random() * 11) + 15;
                    actor.hp = Math.max(0, actor.hp - backlash);
                    broadcastRoomLog(roomId, `⚡ 강력한 힘의 반동이 연화의 육신을 덮칩니다! (-${backlash})`, "combat-msg");
                }
            } else if (cmd.skillName === '징치기') { actor.status.push('도발방어'); broadcastRoomLog(roomId, `🥁 천화가 징을 크게 울려 적들의 시선을 끌고 방어 태세를 갖춥니다!`, "heal-msg"); }
            else { gs.isWaitingForInput = true; broadcastRoomLog(roomId, "정확한 스킬명을 입력해야 합니다.", "guide-msg"); io.to(roomId).emit('turn_start', actor); return; }
        } else if (cmd.action === 'DEFEND') { actor.status.push('방어'); broadcastRoomLog(roomId, `🛡️ ${actor.name}이 방어 태세를 취합니다.`); }
        else if (cmd.action === 'SEARCH') { gs.enemies.forEach(e => { e.status = e.status.filter(s => s !== '은신'); }); broadcastRoomLog(roomId, `🔍 ${actor.name}이 주변을 탐색하여 숨은 적을 찾아냅니다.`); }
        ...
        if (gs.location === '1층 주방 (식당)' && gs.phase === 'COMBAT') {
            ...
            if (polter && polter.hp <= polter.maxHp * 0.5 && !gs.poltergeistState.hasRehidden) {
                ...
                if (isBound) {
                    ...
                    gs.phase = 'STORY_KITCHEN_FIND';
                    broadcastRoomLog(roomId, `💨 **[${polter.name}]**이 비명을 지르며 다시 주방 구석으로 숨어버렸습니다!`, "combat-msg");
                    broadcastRoomLog(roomId, "💡 다시 **[둘러보기]**와 **[탐색]**을 통해 본체를 찾아야 합니다!", "guide-msg");
                    broadcastRoomState(roomId);
                    setTimeout(() => { gs.isWaitingForInput = true; syncInputState(roomId, "▶ '둘러보기' 혹은 '탐색 [장소]'를 입력하세요."); }, 1000);
                    return;
                }
            }
        }
        ...
        if (gs) { gs.isWaitingForInput = true; syncInputState(roomId, "⚠️ 기술/공격 처리 중 오류가 발생했습니다."); }
    }
}
